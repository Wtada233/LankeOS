...
void extract_tar_zst(const fs::path &archive_path, const fs::path &output_dir) {
  ...
  archive_write_disk_set_options(
      ext.get(), ARCHIVE_EXTRACT_TIME | ARCHIVE_EXTRACT_PERM |
                     ARCHIVE_EXTRACT_OWNER | ARCHIVE_EXTRACT_ACL |
  ...
  if (archive_read_open_filename(a.get(), archive_path.c_str(), 10240) !=
      ARCHIVE_OK) {
  ...
  while (true) {
    ...
    if (r < ARCHIVE_OK) {
      ...
      while (true) {
        ...
        if (archive_write_data_block(ext.get(), buff, size, offset) <
            ARCHIVE_OK) {
          const char *err = archive_error_string(ext.get());
          throw LpkgException(
              string_format("error.extract_failed", archive_path.string()) +
...
std::string extract_file_from_archive(const fs::path &archive_path,
  ...
  if (archive_read_open_filename(a.get(), archive_path.c_str(), 10240) !=
      ARCHIVE_OK) {
    throw LpkgException(
        string_format("error.open_file_failed", archive_path.string()) + ": " +
