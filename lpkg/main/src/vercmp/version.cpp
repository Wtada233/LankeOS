...
{
    if (v.empty()) throw LpkgException(string_format("error.invalid_version_format", v));
...
struct Version {
    ...
    Version(const std::string& version_str)
    {
        ...
        if (pre_release_pos != std::string::npos) {
            size_t pre_release_end =
                (build_meta_pos > pre_release_pos) ? build_meta_pos : std::string::npos;
            std::string_view pre_release_sv =
                v_sv.substr(pre_release_pos + 1, pre_release_end - (pre_release_pos + 1));
            std::string pre_release_str(pre_release_sv);
            std::sregex_token_iterator pre_it(pre_release_str.begin(), pre_release_str.end(),
                                              re_dot, -1);
        ...
        if (build_meta_pos != std::string::npos) {
            size_t release_end =
                (pre_release_pos > build_meta_pos) ? pre_release_pos : std::string::npos;
            std::string_view release_sv =
                v_sv.substr(build_meta_pos + 1, release_end - (build_meta_pos + 1));
...
int compare_pre_release_part(const std::vector<std::string>& p1, const std::vector<std::string>& p2,
                             const std::string& v1_str, const std::string& v2_str)
{
    ...
    for (size_t i = 0; i < min_len; ++i) {
        ...
        int n1, n2;
        bool is_num1 =
            !p1[i].empty() && std::all_of(p1[i].begin(), p1[i].end(),
                                          [](unsigned char c) { return std::isdigit(c); });
        bool is_num2 =
            !p2[i].empty() && std::all_of(p2[i].begin(), p2[i].end(),
                                          [](unsigned char c) { return std::isdigit(c); });
...
bool version_compare(const std::string& v1_str, const std::string& v2_str)
{
    ...
    // 2. 补丁后缀比较（pN）— 优先级最高
    if (!v1.patch_suffix.empty() && v2.patch_suffix.empty()) return false;  // v1 > v2
    if (v1.patch_suffix.empty() && !v2.patch_suffix.empty()) return true;   // v1 < v2
    if (!v1.patch_suffix.empty() && !v2.patch_suffix.empty()) {
        ...
        // 字母相同，比较尾部数字
        int num1 = 0, num2 = 0;
        if (v1.patch_suffix.size() > 1) num1 = parse_int(v1.patch_suffix.substr(1), v1_str);
        if (v2.patch_suffix.size() > 1) num2 = parse_int(v2.patch_suffix.substr(1), v2_str);
    ...
    // 4. 预发布比较（-）— 低于基础版（原语义）
    if (!v1.pre_release_part.empty() && v2.pre_release_part.empty()) return true;   // v1 < v2
    if (v1.pre_release_part.empty() && !v2.pre_release_part.empty()) return false;  // v1 > v2
    if (!v1.pre_release_part.empty() && !v2.pre_release_part.empty()) {
        return compare_pre_release_part(v1.pre_release_part, v2.pre_release_part, v1_str, v2_str) <
               0;
    }
