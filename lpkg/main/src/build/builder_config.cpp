...
BuildConfig parse_build_config(const fs::path &json_path) {
  ...
  try {
    ...
    throw LpkgException(
        string_format("error.lankebuild_parse_failed", std::string(e.what())));
  }
  ...
  cfg.sources =
      meta.value(std::string(constants::J_SOURCES), std::vector<std::string>{});
  cfg.work_sources = meta.value(std::string(constants::J_WORK_SOURCES),
  ...
  cfg.provides = meta.value(std::string(constants::J_PROVIDES),
                            std::vector<std::string>{});
  cfg.needed_so = meta.value(std::string(constants::J_NEEDED_SO),
                             std::vector<std::string>{});
