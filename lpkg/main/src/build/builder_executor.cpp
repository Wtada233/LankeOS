 ...
 */
std::string
process_build_script(const fs::path &script_path,
...
void execute_build_phase(const std::string &phase_name,
  ...
  std::string cmd =
      ". " + fs::absolute(processed_script_path).string() + " && " + phase_name;
  int ret = run_shell(cmd, work_dir);
  if (ret != 0) {
    fs::remove(processed_script_path);
    throw LpkgException(string_format("error.build_phase_failed", phase_name,
                                      std::to_string(ret)));
  }
}
