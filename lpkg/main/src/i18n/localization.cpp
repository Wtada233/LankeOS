...
void init_localization() {
  ...
  fs::path relative_l10n_dir =
      exec_dir / ".." / "l10n"; // l10n 目录在构建目录上一级
  fs::path main_l10n_dir =
      exec_dir / ".." / "main" / "l10n"; // main/l10n 目录布局
  fs::path src_l10n_dir =
      exec_dir / ".." / "src" / "l10n"; // 测试目录下的 src/l10n 布局
...
const std::string &get_string(const std::string &key) {
  ...
  if (missing_it == missing_key_placeholders.end()) {
    missing_it =
        missing_key_placeholders.emplace(key, "[MISSING_STRING: " + key + "]")
            .first;
  }
