...
static int handle_command(const std::string& command, const cxxopts::ParseResult& result,
                          const std::string& hash_file, std::function<void()> usage)
{
    if (command == constants::CMD_INSTALL) {
        pre_operation_check(result, usage, 1);
        install_packages(result["packages"].as<std::vector<std::string>>(), hash_file,
        ...
        detail::read_package_metadata(source_dir, pkg_name, pkg_ver, deps, provides, needed_so,
                                      man);

        pack_package(result["output"].as<std::string>(), source_dir, pkg_name, pkg_ver, deps,
                     provides, man, needed_so);
        ...
        auto args = result.count("packages") ? result["packages"].as<std::vector<std::string>>()
        ...
        if (sub == "remove") {
            ...
            for (size_t i = 1; i < args.size(); ++i) {
                fs::path p(args[i]);
                auto root =
                    (p.extension() == constants::EXT_LPKG || p.extension() == constants::EXT_ZST)
        ...
        std::string r;
        if (result.count("packages"))
            if (auto v = result["packages"].as<std::vector<std::string>>(); !v.empty()) r = v[0];
        scan_orphans(r);
