...
void print_usage(const cxxopts::Options &options) {
  ...
  std::cerr << get_string("info.install_desc") << "  "
            << get_string("info.install_opts") << std::endl;
  std::cerr << get_string("info.remove_desc") << "  "
            << get_string("info.remove_opts") << std::endl;
  std::cerr << get_string("info.autoremove_desc") << std::endl;
  std::cerr << get_string("info.upgrade_desc") << "  "
            << get_string("info.upgrade_opts") << std::endl;
  std::cerr << get_string("info.reinstall_desc") << "  "
            << get_string("info.reinstall_opts") << std::endl;
  std::cerr << get_string("info.query_desc") << "  "
            << get_string("info.query_opts") << std::endl;
  std::cerr << get_string("info.man_desc") << std::endl;
  std::cerr << get_string("info.pack_desc") << "  "
            << get_string("info.pack_opts") << std::endl;
...
static int handle_command(const std::string &command,
  ...
  if (command == constants::CMD_INSTALL) {
    ...
    detail::read_package_metadata(source_dir, pkg_name, pkg_ver, deps, provides,
                                  needed_so, man);

    pack_package(result["output"].as<std::string>(), source_dir, pkg_name,
                 pkg_ver, deps, provides, man, needed_so);

  } else if (command == constants::CMD_BUILD) {
    std::string dir = ".";
    if (result.count("packages"))
      if (auto v = result["packages"].as<std::vector<std::string>>();
          !v.empty())
        dir = v[0];
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options(get_string("help.group_general"))(
        "h,help", get_string("info.help_desc"))("v,version",
                                                get_string("help.version"));

    // --- 安装/移除选项 ---
    options.add_options(get_string("help.group_install"))(
        "y,yes", get_string("help.yes_mode"),
        cxxopts::value<bool>()->default_value("false"))(
        "n,no", get_string("help.no_mode"),
        cxxopts::value<bool>()->default_value("false"))(
        "force", get_string("help.force"),
        cxxopts::value<bool>()->default_value("false"))(
        "force-overwrite", get_string("help.force_overwrite"),
        cxxopts::value<bool>()->default_value("false"))(
        "no-hooks", get_string("help.no_hooks"),
        cxxopts::value<bool>()->default_value("false"))(
        "no-deps", get_string("help.no_deps"),
        cxxopts::value<bool>()->default_value("false"))(
    ...
    options.add_options(get_string("help.group_pack"))(
        "o,output", get_string("help.output_file"),
        cxxopts::value<std::string>())(
        "d,directory", get_string("help.pack_source"),
        cxxopts::value<std::string>()->default_value(
            std::string(constants::DEFAULT_PACK_SOURCE)));

    // --- 其他选项 ---
    options.add_options(get_string("help.group_other"))(
        "all", get_string("help.depend_all"),
        cxxopts::value<bool>()->default_value("false"));
    ...
    if (result.count("force-overwrite"))
      Config::instance().set_force_overwrite_mode(
          result["force-overwrite"].as<bool>());
    ...
    if (command == constants::CMD_INSTALL || command == constants::CMD_REMOVE ||
        command == constants::CMD_AUTOREMOVE ||
        command == constants::CMD_UPGRADE ||
