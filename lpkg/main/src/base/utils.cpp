 ...
 */
void log_internal(std::string_view prefix, std::string_view color,
                  std::string_view msg, std::ostream &stream) {
  ...
  if (current_stream_is_tty) {
    stream << color << prefix << constants::COLOR_WHITE << msg
           << constants::COLOR_RESET << std::endl;
  } else {
...
void log_info(std::string_view msg) {
  log_internal(get_string("info.log_prefix"), constants::COLOR_GREEN, msg,
               std::cout);
}
...
void log_warning(std::string_view msg) {
  log_internal(get_string("warning.prefix") + " ", constants::COLOR_YELLOW, msg,
               std::cerr);
}
...
void log_error(std::string_view msg) {
  log_internal(get_string("error.prefix") + " ", constants::COLOR_RED, msg,
               std::cerr);
}
...
void log_progress(const std::string &msg, double percentage, int bar_width) {
  ...
  std::cout << "\r" << constants::COLOR_GREEN << "==> "
            << constants::COLOR_WHITE << msg << " [";
...
DBLock::DBLock() {
  ...
  lock_fd =
      open(Config::instance().lock_file().c_str(), O_CREAT | O_RDWR, 0644);
  if (lock_fd < 0) {
    throw LpkgException(string_format("error.create_file_failed",
                                      Config::instance().lock_file().string()));
  }
...
void ensure_dir_exists(const fs::path &path) {
  if (!fs::exists(path)) {
    ...
    if (!fs::create_directories(path, ec)) {
      throw LpkgException(
          string_format("error.create_dir_failed", path.string()) + ": " +
          ec.message());
    }
...
void ensure_file_exists(const fs::path &path) {
  if (!fs::exists(path)) {
    ...
    if (!file) {
      throw LpkgException(
          string_format("error.create_file_failed", path.string()) + ": " +
          strerror(errno));
    }
  }
}
 ...
 */
void write_set_to_file(const fs::path &path,
  ...
  fs::path tmp_path = path.string() + ".tmp";
  {
    ...
    if (!file.is_open()) {
      throw LpkgException(
          string_format("error.create_file_failed", tmp_path.string()));
    }
...
void cleanup_tmp_dirs() {
  ...
  for (const auto &entry : fs::directory_iterator(tmp_path)) {
    try {
      if (fs::is_symlink(entry.path()) || !entry.is_directory())
        continue;
      const std::string dirname = entry.path().filename().string();
      if (!dirname.starts_with("lpkg_"))
        continue;
      ...
      log_warning(string_format("warning.cleanup_old_tmp_failed",
                                entry.path().string()) +
                  ": " + e.what());
    }
  }
...
void strip_binary(const fs::path &path) {
  ...
  if (!strip_file(path, error_msg)) {
    if (!error_msg.empty()) {
      log_warning(
          string_format("warning.strip_failed", path.string(), error_msg));
    }
  }
}
