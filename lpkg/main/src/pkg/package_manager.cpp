...
//   remove_package → 用户主动删除（写 RM_BEGIN/BACKUP/RM_COMMIT，运行 prerm
//   钩子） rollback       → 事务回滚（直接删文件，不运行钩子，写 ROLLBACK WAL
//   行）
...
static void rollback_installed_package(const std::string &pkg_name) {
  ...
  // ── 阶段 1：恢复旧文件 / 删除新文件 ──────────────────────────────
  //   调用者 rollback_committed_packages 已先执行 restore_owner_overrides()，
  //   从 WAL 中 OWNER_OVERRIDE 条目恢复 force_overwrite 剥夺的所有权，
  //   确保 get_package_files 返回正确的结果。
  auto owned_files = cache.get_package_files(pkg_name);
  for (const auto &f : owned_files) {
    if (f.ends_with('/'))
      continue;
    const fs::path phys =
        Config::instance().root_dir() / fs::path(f).relative_path();
    std::error_code ec;

    const fs::path bak_path =
        phys.string() + std::string(constants::SUFFIX_LPKG_BAK) + pkg_name;
    if (fs::exists(bak_path)) {
      log_info(string_format("info.rollback_restore", f, bak_path.string()));
      TransactionLog::log_raw("RESTORE_FILE " + bak_path.string() +
                              " \xe2\x86\x92 " + phys.string());
      fs::rename(bak_path, phys, ec);
    } else if (fs::exists(phys) || fs::is_symlink(phys)) {
      fs::remove(phys, ec);
    }
...
// 确保 rollback_installed_package 的 get_package_files 能正确返回文件列表。
static void restore_owner_overrides() {
  const fs::path log_path = Config::instance().lock_dir() / "transaction.log";
  std::ifstream f(log_path);
  if (!f.is_open())
    return;

  static constexpr std::string_view PREFIX = "OWNER_OVERRIDE ";
  ...
  while (std::getline(f, line)) {
    ...
    if (pos == std::string::npos)
      continue;
    std::string rest = line.substr(pos + PREFIX.size());
    auto sp = rest.find(' ');
    if (sp == std::string::npos)
      continue;
    overrides.emplace_back(rest.substr(0, sp), rest.substr(sp + 1));
  }

  // 逆序恢复：后剥夺的先恢复（栈语义：A剥夺base，B剥夺A → 先恢复A再恢复base）
  for (auto it = overrides.rbegin(); it != overrides.rend(); ++it)
    Cache::instance().add_file_owner(it->first, it->second);
}

// ── 批量回滚：撤销同一批次中所有已成功安装的包 ────────────────────
static void rollback_committed_packages(std::vector<std::string> &installed) {
  // 先恢复 force_overwrite 剥夺的所有权（从 WAL OWNER_OVERRIDE 读取）
  restore_owner_overrides();
  ...
  for (auto it = installed.rbegin(); it != installed.rend(); ++it) {
    try {
      ...
      log_warning(
          string_format("warning.batch_rollback_failed", *it, e.what()));
    }
  }
  installed.clear();
}
 ...
 */
static void with_batch_transaction(std::vector<std::string> &success,
                                   const std::string &wal_tag, size_t total,
                                   const std::function<void()> &body) {
  TransactionLog::log_raw("BEGIN_PKGS " + std::to_string(total));

  try {
    // 测试断点：BEGIN_PKGS 后立即检查（必须在 try 内，确保 catch 补
    // COMMIT_PKGS）
    ...
    // 批次已提交，安全删除所有延迟的 .lpkg_bak 备份文件
    InstallationTask::process_deferred_cleanups();
  } catch (...) {
    // 回滚同一批次中所有已成功安装的包。
    ...
    // 回滚已从 .lpkg_bak 恢复旧文件（若存在），丢弃延迟清理列表
    InstallationTask::discard_deferred_cleanups();
...
void install_packages(const std::vector<std::string> &pkg_args,
  ...
  //   2. 若元数据与索引不匹配 → 更新仓库信息 → 回滚已安装包 → 重解析依赖 →
  //   从头重试
  //   3. 匹配 → 执行 InstallationTask::run()（解压 → 依赖检查 → 冲突检测 →
  //   写文件）
...
void remove_package(const std::string &pkg_name, bool force, bool wrap_in_txn) {
  ...
  // SIGINT 检查：在文件操作开始前允许安全中断，此时仅写入了 WAL 日志头
  if (sigint_graceful.load())
    throw LpkgException(get_string("info.sigint_aborted"));
 ...
 *   - 若最新版本 > 当前版本 → 创建
 * InstallationTask（传入旧版本号以触发文件清理）
