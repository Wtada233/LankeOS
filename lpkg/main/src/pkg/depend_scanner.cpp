...
void collect_transitive_rdeps(const std::string &root_pkg,
  ...
  if (!visited.insert(root_pkg).second)
    return;
...
void resolve_transitive_deps(const std::string &pkg_name,
                             const std::string &version_spec, DepMap &plan,
                             std::set<std::string> &visited, Repository &repo) {
  if (visited.contains(pkg_name) || plan.contains(pkg_name))
    return;
  ...
  auto pkg_info =
      (version_spec == constants::VER_LATEST || version_spec.empty())
  ...
  plan[pkg_info->name] = {pkg_info->name, pkg_info->version, already,
...
void build_remove_tree_local(ScanNode &node, const std::string &node_name,
  ...
  for (const auto &rdep : rdeps) {
    if (rdep == node_name || !affected.contains(rdep))
      continue;
    affected.erase(rdep);
    ...
    if (show_all) {
      ...
      if (fs::exists(df)) {
        ...
        while (std::getline(f, l)) {
          ...
          if (dn.empty() || !seen.insert(dn).second)
            continue;
          ScanNode k;
          k.name = dn;
...
void build_remove_tree_repo(
  ...
  if (it == rev.end())
    return;
  for (const auto &dep : it->second) {
    if (dep == node_name || !affected.contains(dep))
      continue;
    affected.erase(dep);
    ScanNode child;
    child.name = dep;
 ...
 */
void build_install_tree(ScanNode *parent, const std::string &parent_name,
                        const DepMap &plan, std::set<std::string> &seen,
                        Repository &repo, bool show_all) {
  ...
  for (const auto &dep : pit->second.deps) {
    ...
    child.status =
        dit->second.already_installed ? ScanStatus::KEEP : ScanStatus::INSTALL;
    child.reason =
        dit->second.already_installed ? "already installed" : "dependency";
...
ScanNode scan_install_tree(const std::string &pkg_name, bool show_all) {
  ...
  root.status =
      it->second.already_installed ? ScanStatus::KEEP : ScanStatus::INSTALL;
  root.reason =
      it->second.already_installed ? "already installed" : "target package";
...
ScanNode scan_install_from_file(const fs::path &lpkg_path, bool show_all) {
  ...
  root.reason =
      not_installed ? "target package (local)" : "already installed (local)";
  ...
  for (const auto &dep : deps) {
    ...
    child.status =
        pit->second.already_installed ? ScanStatus::KEEP : ScanStatus::INSTALL;
    child.reason =
        pit->second.already_installed ? "already installed" : "dependency";
...
void print_subtree(const ScanNode &node, const std::string &prefix) {
  for (size_t i = 0; i < node.children.size(); ++i) {
    ...
    std::cout << prefix << (last ? "└── " : "├── ")
              << status_color(child.status) << child.name << " ("
              << child.version << ") "
