 ...
 */
static bool strip_elf_rel_object(Elf *in_elf, const GElf_Ehdr &ehdr,
                                 size_t shstrndx, int elf_class,
    ...
    if (in_data) {
      *out_data = *in_data;
      ...
      if (new_shdr.sh_type == SHT_SYMTAB) {
        size_t sym_count =
            out_data->d_size / gelf_fsize(out_elf, ELF_T_SYM, 1, EV_CURRENT);
 ...
 */
static bool strip_elf_exec_dyn(Elf *in_elf, const GElf_Ehdr &ehdr,
                               size_t shstrndx, int elf_class,
  ...
  while ((scn = elf_nextscn(in_elf, scn)) != nullptr) {
    ...
    if (!(shdr.sh_flags & SHF_ALLOC)) {
      if (name != ".shstrtab" && shdr.sh_type != SHT_STRTAB)
        keep = false;
    }
    // 丢弃符号表、字符串表、注释和调试信息
  ...
  size_t shentsize =
      (elf_class == ELFCLASS64) ? sizeof(Elf64_Shdr) : sizeof(Elf32_Shdr);
  GElf_Ehdr updated_ehdr = ehdr;
  ...
  std::memcpy(output_data.data(), input_data.data(),
              std::min(headers_size, input_data.size()));
  ...
  auto write_shdr = [&]<typename Shdr>(size_t i) {
    Shdr *out = reinterpret_cast<Shdr *>(output_data.data() +
                                         updated_ehdr.e_shoff + i * shentsize);
 ...
 * 根据 ELF 类型分派到对应的子函数
 */
bool strip_elf_data(const std::vector<uint8_t> &input_data,
                    std::vector<uint8_t> &output_data,
                    [[maybe_unused]] std::string &error_msg) {
  if (elf_version(EV_CURRENT) == EV_NONE)
    return false;

  Elf *in_elf = elf_memory(
  ...
  if (ehdr.e_type == ET_REL) {
    result =
        strip_elf_rel_object(in_elf, ehdr, shstrndx, elf_class, output_data);
  } else {
    result = strip_elf_exec_dyn(in_elf, ehdr, shstrndx, elf_class, input_data,
                                output_data);
  }
  elf_end(in_elf);
  return result;
...
bool process_elf(const fs::path &path, std::string &error_msg) {
  ...
  if (!strip_elf_data(buffer, output_buffer, error_msg))
    return false;

  std::ofstream os(path, std::ios::binary | std::ios::trunc);
  if (!os)
    return false;
  os.write(reinterpret_cast<const char *>(output_buffer.data()),
           output_buffer.size());
  return true;
}
