...
**结论**：`reverse_execute` 始终跳过 `RESTORE_*`、`REMOVE_*`、`ROLLBACK`、`END`、`COMMIT`、`BEGIN`、`RM_*`、`CLEANUP` 元数据/审计行以及 DB `:batch-start` 条目。只处理正向操作行（BACKUP、COPY、NEW、NEW_DIR、DB/DBNEW/DBRM 非:batch-start）。
