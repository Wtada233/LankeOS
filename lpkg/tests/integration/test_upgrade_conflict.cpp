...
{
    std::string p1 =
        create_pkg("cleanup_test", "1.0", {{"usr/bin/file1", "/"}, {"usr/bin/file2", "/"}});
    install_packages({p1});
    ...
    EXPECT_TRUE(fs::exists(test_root / "usr/bin/file1"));
    EXPECT_FALSE(fs::exists(test_root / "usr/bin/file2"))
        << "Obsolete file was not removed during upgrade!";
}
...
TEST_F(ComprehensiveTest, AutoremoveHandlesVirtualChains)
{
    ...
    EXPECT_TRUE(fs::exists(test_root / "usr/lib/libssl.so"))
        << "Provider of virtual dependency was incorrectly autoremoved!";
}
