...
TEST_F(UpgradeDepsResolutionTest, UpgradeDiscoversNewDependency)
{
    ...
    std::string p1 =
        create_pkg("app", "1.0", {{"usr/bin/app", "app v1"}});  // 只有 app 自身，不捆绑任何共享库
...
TEST_F(UpgradeDepsResolutionTest, BundledLibTransitionsToExternalDep)
{
    ...
    EXPECT_TRUE(owners.contains("libprovider")) << "libhelper.so.1 应归属于 libprovider";
...
{
    std::string p1 = create_pkg("app", "1.0", {{"usr/bin/app", "app v1"}});
    ASSERT_NO_THROW(install_packages({p1}));
    Cache::instance().load();
    ASSERT_EQ(Cache::instance().get_installed_version("app"), "1.0");
...
{
    std::string pa1 = create_pkg("appA", "1.0", {{"usr/bin/appA", "A v1"}});
    std::string pb1 = create_pkg("appB", "1.0", {{"usr/bin/appB", "B v1"}});
...
TEST_F(UpgradeDepsResolutionTest, HoldPreservedAfterUpgrade)
{
    ...
    Cache::instance().load();
    EXPECT_TRUE(Cache::instance().is_held("app")) << "升级后包应保持 held 状态";
}
