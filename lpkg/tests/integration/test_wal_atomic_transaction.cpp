...
TEST_F(RecoveryTest, RecoverRestoresBackedUpFiles)
{
...

    {
        ...
        w << "BEGIN_PKGS 1\n"
          ...
          << "COPY " + (test_root / "tmp/crashed_pkg.lpkgtmp").string() + " \xe2\x86\x92 "
          << orig.string() << "\n"
    ...
    std::string wal_content((std::istreambuf_iterator<char>(wf)), std::istreambuf_iterator<char>());
...
TEST_F(RecoveryTest, RecoverWithRestoreAuditLines)
{
...

    {
        ...
        w << "BEGIN_PKGS 2\n"
          << "BEGIN A 1.0\n"
          << "BACKUP " << orig.string() << " \xe2\x86\x92 " << bak.string() << "\n"
          << "COPY " + (test_root / "tmp/a").string() + " \xe2\x86\x92 " << orig.string() << "\n"
          << "COMMIT A 1.0\n"
          << "END A 1.0\n"
          << "BEGIN B 1.0\n"
          << "ROLLBACK B 1.0\n"
...
TEST_F(SecondaryRollbackTest, InterruptedInstallFileRecovery)
{
    ...
    fs::path bak_file(orig_file.string() + std::string(constants::SUFFIX_LPKG_BAK) +
                      "half_installed");
...
TEST_F(SecondaryRollbackTest, BatchWithRollbackFullRecovery)
{
...

    {
        ...
        w << "BEGIN_PKGS 3\n"
          ...
          << "COPY " + (test_root / "tmp/a").string() + " \xe2\x86\x92 " << a_file.string() << "\n"
          << "COMMIT A 1.0\n"
          << "END A 1.0\n"
          << "DB /var/lib/lpkg/pkgs A:installed\n"
          << "BEGIN B 1.0\n"
          << "NEW " << b_new.string() << "\n"
          << "COPY " + (test_root / "tmp/b").string() + " \xe2\x86\x92 " << b_new.string() << "\n"
          << "ROLLBACK B 1.0\n"
          << "END B 1.0\n"
