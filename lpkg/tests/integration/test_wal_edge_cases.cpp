...
TEST_F(WalEdgeCaseTest, TrimLargeCompletedBatch)
{
    ...
    wal += "BEGIN_PKGS 1\n";
    for (int i = 0; i < 20; ++i)
        wal += "BACKUP /file" + std::to_string(i) + " \xe2\x86\x92 /file" + std::to_string(i) +
               ".bak\n";
    wal += "COMMIT_PKGS\n";
    wal += "BEGIN_PKGS 1\nBEGIN active 1.0\n";
    write_wal(wal);
...
{
    auto op =
        wal::parse_op("BACKUP /path/with spaces/file \xe2\x86\x92 /path/with spaces/file.bak");
...
TEST_F(WalEdgeCaseTest, MultipleRecoverCallsIdempotent)
{
    ...
    write_wal(
        ...
        "BEGIN mr 1.0\n"
        "BACKUP " +
        f1.string() + " \xe2\x86\x92 " + b1.string() + "\n");
