...
TEST_F(ComplexAtomicTest, RecursiveRemoveRollbackDeepChain)
{
    ...
    fs::path root_bak = root_bin.string() + std::string(constants::SUFFIX_LPKG_BAK) + "cr_root";
    fs::path mid1_bin = test_root / "usr/bin/cr_mid1";
    fs::path mid1_bak = mid1_bin.string() + std::string(constants::SUFFIX_LPKG_BAK) + "cr_mid1";
    ...
    write_wal(
        ...
        "BACKUP " +
        root_bin.string() + " \xe2\x86\x92 " + root_bak.string() + "\n"
        // 在此崩溃
    );
...
TEST_F(ComplexAtomicTest, UpgradeAbiBreakRollback)
{
    ...
    // 发布 lib v2 —— 提供 libABI.so.2（不再提供 .so.1）
    {
        ...
        std::ofstream(work / "content" / "usr" / "lib" / "libABI.so.2") << "v2\n";
        {
...
TEST_F(ComplexAtomicTest, RecoveryWithGarbageBetweenValidOps)
{
    ...
    fs::path a_bak = a_file.string() + std::string(constants::SUFFIX_LPKG_BAK) + "garb_A";
    ...
    write_wal(
        ...
        "BEGIN garb_A 1.0\n"
        "BACKUP " +
        a_file.string() + " \xe2\x86\x92 " + a_bak.string() +
        "\n"
...
TEST_F(ComplexAtomicTest, RecoveryBackupOnlyNoCopy)
{
    ...
    fs::path bak = file.string() + std::string(constants::SUFFIX_LPKG_BAK) + "bak_only";
    ...
    write_wal(
        ...
        "BEGIN bak_only 1.0\n"
        "BACKUP " +
        file.string() + " \xe2\x86\x92 " + bak.string() + "\n");
...
TEST_F(ComplexAtomicTest, MixedBackupCopyNewRecovery)
{
    ...
    fs::path old_bak = old_file.string() + std::string(constants::SUFFIX_LPKG_BAK) + "mixed";
    ...
    write_wal(
        ...
        "BEGIN mixed 1.0\n"
        "BACKUP " +
        old_file.string() + " \xe2\x86\x92 " + old_bak.string() +
        "\n"
        "NEW " +
        new_file.string() +
        "\n"
