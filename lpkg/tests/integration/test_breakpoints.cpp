...
TEST_F(BreakpointTest, FileBackupPowerLoss_BeforeRename)
{
    ...
    write_wal(
        "BEGIN_PKGS 1\nBEGIN pt 1.0\n"
        "BACKUP " +
        orig.string() + " \xe2\x86\x92 " + bak.string() + "\n");
...
TEST_F(BreakpointTest, CopyPowerLoss_BeforeRename)
{
    ...
    write_wal(
        "BEGIN_PKGS 1\nBEGIN np 1.0\n"
        "COPY " +
        tmp.string() + " \xe2\x86\x92 " + dst.string() + "\n");
...
TEST_F(BreakpointTest, SecondaryRollbackAllDoneBeforeCommit)
{
    ...
    write_wal(
        ...
        "BEGIN B 1.0\nROLLBACK B 1.0\nEND B 1.0\n"
        "RESTORE_FILE " +
        abak.string() + " \xe2\x86\x92 " + a.string() +
        "\n"
        "ROLLBACK A 1.0\nEND A 1.0\n"
        "DB " +
        pkgs + " :batch-start\n");
...
TEST_F(BreakpointTest, SecondaryRollbackAfterRestoreFileRm)
{
    ...
    write_wal(
        ...
        "ROLLBACK B 1.0\nEND B 1.0\n"
        "RESTORE_FILE " +
        a_bak.string() + " \xe2\x86\x92 " + a_orig.string() +
        "\n"
        "RESTORE_FILE_RM " +
        b_new.string() + "\n");
    ...
    EXPECT_FALSE(fs::exists(a_orig))
        << "File restored by RESTORE_FILE is re-deleted by COPY reverse during recover";
...
TEST_F(BreakpointTest, MixedNewCopyBackupReverse)
{
    ...
    write_wal(
        "BEGIN_PKGS 1\nBEGIN mix 1.0\n"
        "BACKUP " +
        old_f.string() + " \xe2\x86\x92 " + old_bak.string() +
        "\n"
        "NEW " +
        new_f.string() +
        "\n"
...
TEST_F(BreakpointTest, BatchRemoveCrashMidwayRestoresAllPackages)
{
    ...
    for (auto& n : {"rmA_crash", "rmB_crash", "rmC_crash"}) {
        ...
        EXPECT_TRUE(fs::exists(test_root / "usr/bin" / n)) << n << " binary should exist";
    }
    ...
    fs::path pA_bak = pA_bin.string() + std::string(constants::SUFFIX_LPKG_BAK) + "rmA_crash";
    fs::path pB_bin = test_root / "usr/bin/rmB_crash";
    fs::path pB_bak = pB_bin.string() + std::string(constants::SUFFIX_LPKG_BAK) + "rmB_crash";
    ...
    write_wal(
        ...
        "BACKUP " +
        pC_bin.string() + " \xe2\x86\x92 " + pC_bin.string() +
        ".lpkg_bak_rmC_crash\n"
        ...
        "BACKUP " +
        pB_bin.string() + " \xe2\x86\x92 " + pB_bak.string() +
        "\n"
        "RM_COMMIT rmB_crash 1.0\n"
        "RM_END rmB_crash 1.0\n"
        "RM_BEGIN rmA_crash 1.0\n"
        "BACKUP " +
        pA_bin.string() + " \xe2\x86\x92 " + pA_bak.string() + "\n"
