...
TEST_F(WalCoreTest, ParseOpRestoreDb)
{
    auto op = wal::parse_op(
        "RESTORE_DB /var/lib/lpkg/pkgs.lpkg_db_bak_before:glibc:installed → /var/lib/lpkg/pkgs");
...
TEST_F(WalCoreTest, ReverseExecuteDbNewWithBackup)
{
    ...
    fs::path bak_path = fs::path(db_path.string() + ".lpkg_db_bak_before:bash:installed");
...
TEST_F(WalCoreTest, ReverseExecuteDbRmWithBackup)
{
    ...
    fs::path bak_path = fs::path(db_path.string() + ".lpkg_db_bak_before:vim:removed");
...
{
    for (auto t : {wal::WALOpType::BEGIN_PKGS, wal::WALOpType::BACKUP, wal::WALOpType::COPY,
                   wal::WALOpType::DB, wal::WALOpType::RESTORE_FILE, wal::WALOpType::RM_BEGIN}) {
