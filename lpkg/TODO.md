...
- **I-2**: 批次内每个包安装/移除完成后：`.lpkg_bak` 保留，`.lpkgtmp` 已清，内存 COMMIT 已写，DB 已落盘（带 WAL 保护）。
...
- **I-FSYNC-2**: DB 文件写入顺序：备份原文件 → fsync 备份 → 写 WAL → fsync WAL → 写 .tmp → fsync .tmp → rename .tmp → fsync 父目录。
...
| | `RM_DIR <path>` | 目录已删除 | 写后 fsync |
| | `RM_BAK_CLN <path>` | 目录内 bak 已清扫 | 写后 fsync |
...
| `<pkg>:rolled-back` | 包 `<pkg>` 回滚完成后的状态 |
...
```
 1. 备份：rename old → .lpkg_db_bak_before:<milestone>
 2. fsync：备份文件所在的父目录
 3. WAL：写入 DB <path> <milestone>
 4. fsync：WAL 文件
...
| 断电点 | 系统状态 | 恢复行为 |
|--------|---------|---------|
| 1-2 之间 | 原文件已被 rename 到备份，WAL 无记录 | `recover_packages()` 状态机跳过（无 DB WAL 条目）。`cleanup_db_backups()` 清理孤备份。DB 文件不存在（是新建场景）或丢失（是修改场景）。**== 危险 ==** |
| 2-3 之间 | 同上，备份存在但 WAL 未写 | 同上 |
| 3-4 之间 | 备份 + WAL 有记录，但 WAL 行可能未持久化 | 若 WAL 行未持久化：跳过。若已持久化：restore 从备份恢复 |
| 4-5 之间 | 备份 + 持久化 WAL，.tmp 不存在 | restore 成功 |
| 5-6 之间 | 备份 + WAL + .tmp（可能不完整） | restore 从备份恢复（.tmp 不完整不影响） |
| 6-7 之间 | 备份 + WAL + 完整 .tmp | restore 从备份恢复 |
| 7-8 之间 | 备份 + WAL + 新文件 + 旧备份仍在 | restore 从备份恢复 |

**修正 1-2 的危险区间**：将第一步改为先写 WAL 再备份？不行，那样 WAL 记录存在但备份不完整。

**正确顺序（v2 修正）**：

```
 1. WAL：写入 DB <path> <milestone>          ← 先记录意图
 2. fsync：WAL 文件
...
| 断电点 | 系统状态 | 恢复行为 |
|--------|---------|---------|
| 1-2 之间 | WAL 行未持久化，原文件未改 | 恢复时无 DB 条目 → 跳过。**安全** |
| 2-3 之间 | WAL 持久化，未备份，原文件完好 | `reverse_execute` 找到 DB 条目，检查 `.lpkg_db_bak_before:<milestone>` 不存在。因为是已有文件（非 DBNEW），还原无法进行——怎么办？**

这里出现新问题：WAL 承诺了"修改后的状态是 X"，但备份不存在，恢复不了。解决方案：**在 reverse_execute 中处理"有 DB 条目但备份不存在"的情况**：

- 如果是 `DB`（文件已存在）：WAL 条目存在但备份不存在 → 说明备份步骤（3）未执行，原文件未改。和 DBNEW 一样处理：不做 restore（原文件仍在，内容正确——是修改前的状态，但这是离目标最近的可用状态）。
- 如果是 `DBNEW`（新文件）：备份不存在 → 文件是新建的，没有旧版本。删除文件。
- 如果是 `DBRM`（删除文件）：备份不存在 → 文件已被删除（或未创建）。
...
```
 1. 备份：rename old → .lpkg_db_bak_before:<milestone>
 2. fsync：备份父目录
 3. fsync：原路径父目录（确保 rename 完成）
 4. WAL：写入 DB <path> <milestone>
 5. fsync：WAL
 6. 写 .tmp
 7. fsync：.tmp
 8. rename .tmp → <path>
...
| 断电点 | 恢复行为 |
|--------|---------|
| 1-2 之间 | 可能 old→bak rename 完成但未 fsync。WAL 无记录。文件系统可能恢复原状态或保持 renamed。最坏情况：文件丢失，bak 存在但 WAL 无记录。→ `cleanup_db_backups()` 清理。 |
| 2-4 之间 | backup 已 fsync，WAL 未写。备份存在但 WAL 无对应条目。→ `cleanup_db_backups()` 清理。原文件被 renamed 走了。→ **必须处理这种情况**。 |

这个问题本质上是：**WAL 必须先写再操作**（write-ahead 的定义），但备份也需要在 WAL 之前完成（否则备份时崩溃 WAL 承诺了不存在的备份）。

**最终决定：采用 write-ahead 优先 + 恢复时 fallback**
...
恢复时对 `DB` 条目：
- `.lpkg_db_bak_before:<milestone>` 存在 → rename 回（恢复旧内容）
- `.lpkg_db_bak_before:<milestone>` 不存在（备份未完成）→ **说明步骤 3 未执行，原文件还在**。不操作，跳过（文件内容还是旧的一致状态）。

对 `DBNEW` 条目：
- 备份不存在（新文件没有旧版本）→ 如果文件存在，删除
- 备份存在（罕见：rename old → bak 时 old 是空文件或占位）→ rename 回

对 `DBRM` 条目：
- 备份不存在（文件已删或未备份）→ 跳过
- 备份存在 → rename 回

**结论**：顺序 1→2→3→4→5→6→7→8 是最安全的。恢复函数必须能处理"WAL 条目存在但备份不存在"的情况。
...
- 2-3 之间：完整 .lpkgtmp，WAL 无记录。恢复时遇到 COPY 条目吗？没有。.lpkgtmp 残留由 `cleanup_db_backups` 或下次安装清理。
...
reverse_execute 遇到 RM_DIR：
  → create_directories(op.arg1)（重建目录）
```
...
struct DbMilestone {
    ...
    std::string state; // "installed" | "removed" | "rolled-back" | "batch-start"
...
├── cleanup_backups()
│   (不移除 .bak！批次模式延迟到 COMMIT_PKGS 后清理)
├── WAL: END <pkg> <ver>               ← fsync
│
└── return ✓
```
...
│     释放所有权
│     if 最后持有者:
│       清扫 dir 内 .lpkg_bak_<pkg>
│       WAL: RM_BAK_CLN <path>           ← fsync
│       fs::remove(dir)
│       WAL: RM_DIR <path>               ← fsync
│
...
| `RM_DIR` | create_directories | fsync 父目录 | `RESTORE_DIR <path>` | 已存在→跳过 |
| `RM_BAK_CLN` | skip | - | - | - |
...
| 是否调用 cleanup_db_backups | ✅ 是 | 清理孤备份 |
...
- [ ] **1.1 定义 `DbMilestone` 类型**（`wal_op.hpp`）
- [ ] **1.2 重写 `Cache::write` 使用 write-ahead 顺序**
  - 顺序：WAL → fsync → 备份 → fsync → .tmp → fsync → rename → fsync
  - 备份命名：`.lpkg_db_bak_before:<milestone>`
  - `reverse_execute` 处理"WAL 条目存在但备份不存在"的情况
- [ ] **1.3 实现 WAL 原子写入类 `WalWriter`**
  - 每行 O_APPEND + write + fsync
  - log_raw 和 log_raw_no_fsync 变体（no_fsync 用于非关键路径）
  - 返回写入是否成功
- [ ] **1.4 实现 `parse_op`（新格式 + 兼容旧格式）**
- [ ] **1.5 实现 `reverse_execute`**
  - 跳过 RESTORE_*/REMOVE_*/元数据行
  - 跳过 :batch-start DB 条目
  - 每步写 RESTORE_* 审计
  - 里程碑目标支持
- [ ] **1.6 实现 `extract_current_batch_ops`**
- [ ] **1.7 实现 `batch_rollback`**
  - 清理 cache → reverse_execute → DB :batch-start → ROLLBACK/END → COMMIT_PKGS

### 第 2 阶段：安装事务

- [ ] **2.1 实现 `run_batch_transaction` 模板函数**
- [ ] **2.2 重构 `install_packages`**
  - 使用 `run_batch_transaction`
  - 每包后 `Cache::write(pkg + ":installed")`
- [ ] **2.3 重构 `InstallationTask`**
  - 所有 WAL 操作使用 write-ahead 顺序
  - 包级 catch 写 ROLLBACK/END + 文件回滚 + RESTORE_* 审计
  - 延迟 .bak 清理（批次模式）

### 第 3 阶段：移除事务

- [ ] **3.1 重构 `remove_package`**
  - 使用 `run_batch_transaction`
  - catch 调 `batch_rollback`（不是 recover_packages）
- [ ] **3.2 重构 `remove_package_recursive`**

### 第 4 阶段：升级事务

- [ ] **4.1 重构 `upgrade_packages`**
  - 升级回滚正确恢复旧版本
  - REMOVE_OLD 在回滚中正确处理

### 第 5 阶段：rec、trim、二次回滚

- [ ] **5.1 实现 `recover_packages`**
  - 状态机 + 跳过 RESTORE_* 和 :batch-start
- [ ] **5.2 实现 `trim_completed`**
- [ ] **5.3 二次回滚测试**
  - 在 rollback 各步骤注入断电，验证恢复一致性

### 第 6 阶段：测试

- [ ] **6.1 每个 fsync 点的断电模拟测试**
- [ ] **6.2 逆向操作幂等性测试**
- [ ] **6.3 里程碑链式恢复测试**
- [ ] **6.4 二次回滚幂等性测试**
- [ ] **6.5 各场景集成测试**
...
成功移除:
  BEGIN_PKGS 1 → RM_BEGIN → BACKUP... → DBRM... → DB... → RM_COMMIT → RM_END → COMMIT_PKGS
