...
//!   `IGNORE_CHK_INTROSPECTION` / `IGNORE_CHK_VAPI` / `IGNORE_CHK_HOOK`：包级**豁免**——`farm chk full`
//!   及各 chk（qml/pkgconf/pkg-err/introspection/vapi/hook/abi）对带对应 flag 的包跳过该检則（承认已知/
//!   有意为之）。这些 flag 不影响构建序，只被 chk 消费；在此注册以免 build 解析时误报"未知 flag"。
